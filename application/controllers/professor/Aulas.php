...
class Aulas extends CI_Controller {
  ...
  public function index($id)
  {
    ...
    if (isset($data['cur'][0]['id'])) {
      ...
      $this->load->view('professor/cursos/listar_aulas', $data);
    } else {
    ...
    if (isset($data['aula'][0]['id'])) {
      ...
      if (isset($curso[0]['id'])){
        ...
        if ($this->model->update('aula', $upd, $id)){
          gera_aviso('sucesso', 'Aula ativada com sucesso.', 'professor/cursos/aulas/'.$curso[0]['id'].'-'.$curso[0]['nome']);
        } else {
          gera_aviso('erro', 'Falha ao ativar a aula, tente novamente.', 'professor/cursos/aulas/'.$curso[0]['id'].'-'.$curso[0]['nome']);
        }
      } else {
        gera_aviso('erro', 'Curso não encontrado.', 'professor/cursos');
      }
    } else {
      gera_aviso('erro', 'Aula não encontrada.', 'professor/cursos');
    }
  }
  ...
  public function desativar_aula($id)
  {
    ...
    if (isset($data['aula'][0]['id'])) {
      ...
      if (isset($curso[0]['id'])){
        $upd = ['ativo' => 0];
        if ($this->model->update('aula', $upd, $id)){
          gera_aviso('sucesso', 'Aula desativada com sucesso.', 'professor/cursos/aulas/'.$curso[0]['id'].'-'.$curso[0]['nome']);
        } else {
          gera_aviso('erro', 'Falha ao desativar a aula, tente novamente.', 'professor/cursos/aulas/'.$curso[0]['id'].'-'.$curso[0]['nome']);
        }
      } else {
        gera_aviso('erro', 'Curso não encontrado.', 'professor/cursos');
      }
  ...
  public function update_aulaAjax($id)
  {
    ...
    if (isset($aula[0]['id']) && isset($curso[0]['id'])) {
      ...
      if ($att) {
        ...
        $arquivoAnt = $this->input->post('arqv_ant');
  ...
  public function remover_aula($id)
  {
    ...
    if (isset($admin[0]['id'])) {
      if ($this->model->remove("aula", $id)) {
        ...
        gera_aviso('sucesso', 'Aula removida com sucesso.', 'professor/cursos/aulas/' . $crs[0]['id'] . '-' . $crs[0]['nome']);
      } else {
        gera_aviso('erro', 'Erro ao remover a aula.', 'professor/cursos');
      }
    } else {
      gera_aviso('erro', 'Aula não encontrada.', 'professor/cursos');
    }
