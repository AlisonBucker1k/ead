            ...
            <div class="col-md-6 mb-24pt mb-md-0">
                <form action="<?php echo base_url(); ?>professor/cursos/update/<?= $cur[0]['id']; ?>" enctype="multipart/form-data" method="POST">
                        ...
                        <div class="input-group input-group-merge">
                            <select id="professor" name="id_professor" class="form-control" placeholder="Professor" required>
                                <?php foreach ($professores as $prof) { ?>
                                    <option value="<?= $prof['id']; ?>" <?php if ($prof['id'] == $cur[0]['id_professor']){ echo 'selected'; } ?> >
                                        <?= $prof['nome']; ?>
                                    </option>
                                <?php } ?>
                            </select>
                    ...
                    <div class="form-group mb-24pt">
                        ...
                        <div class="input-group input-group-merge">
                            <input id="venda" type="number" step="0.01" name="venda" value="<?=$cur[0]['venda'];?>" class="form-control form-control-prepended" required placeholder="0.00">
                        ...
                        <div class="alert alert-soft-warning">
                            <div class="d-flex flex-wrap align-items-start">
                                ...
                                <div class="flex" style="min-width: 180px">
                                    ...
                                    <?php echo '<a href="' . site_url('professor/cursos/aulas/' . $ent['id'] . '-' . rawurlencode($ent['nome'])) . '">'; ?>
                                        <button class="btn btn-outline-primary btn-rounded" title="Editar aulas"><i class="fas fa-graduation-cap"></i> Cadastrar / Editar Aulas</button>
                                    </a>
                                    <?php echo '<a href="' . site_url('admin/cursos/provas/' . $ent['id'] . '-' . rawurlencode($ent['nome'])) . '">';  ?>
                                        <button class="btn btn-outline-primary btn-rounded" title="Editar provas"><i class="fas fa-id-card-alt"></i> Cadastrar / Editar Provas</button>
                                    </a>
                                    <?php echo '<a href="' . site_url('admin/cursos/tarefas/' . $ent['id'] . '-' . rawurlencode($ent['nome'])) . '">'; ?>
                                        <button class="btn btn-outline-primary btn-rounded" title="Editar aulas"><i class="far fa-id-card"></i> Cadastrar / Editar Tarefas</button>
                                    </a>
                                    </small>
                                </div>
                            </div>
