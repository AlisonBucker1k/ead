    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                    <input type="hidden" id="tipo-id-tarefa" value="" >
                    <div class="form-group">
                        <label class="form-label" for="name">Questão: *</label>
                        <div id="questao-tarefas" style="height: 150px;" class="mb-0" >
                        </div>
                    </div>
                    <hr>
                    <h5>Escolhas</h5>
                    <div class="card mb-4">
                        <div id="escolhas-tarefas" class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <div id="explica-tarefas" style="height: 150px;" class="mb-0">
                        </div>
...
<script>
    ...
    function validaCampos(){
        ...
        if (text1.replace(/\s/g,"") == ""){
            $('#questao-tarefas').after(`<div class="label-erro text-danger">Preencha a questão!</div>`);
            retorno = false;
        }

        if (!$('#add_exercicio input[name="correta"]:checked').length){
            $('#escolhas-tarefas').append(`<div class="label-erro text-danger">Selecione uma escolha correta!</div>`);
            retorno = false;
        }
        return retorno;
    }
    ...
    function templateTarefaAdmin(id, num, questao, respostas, explicacao, inputs=false, edit=false, valor=""){
        var respostas_html = '';
        ...
        if (inputs){
            ...
            ipthtml += '<input type="hidden" class="questaoAt" name="questao_'+id+'" value="'+iptquestao+'" />';
            ipthtml += '<input type="hidden" class="explicacaoAt" name="explicacao_'+id+'" value="'+iptexplic+'" />';
        }
        ...
        var ulTemplate = `${ulcima}
                ...
                <li class="list-group-item d-flex">
                    ...
                    <span class="text-muted mx-12pt">${valor}</span>
                    <div class="dropdown">
    ...
    function addQuestao(id, questao, escolhas, explicacao, idd){
        if (idd != ''){
            var sendnum = $(idd + ' .card-title').html().replace('Questão ', '');
            var senderId = idd.replace('#questao-', '');
            var template = templateTarefaAdmin(senderId, sendnum, questao, escolhas, explicacao, true, true);
            $(idd).html(template);
        } else {
            var num = $('.questoes').length + 1;
            var template = templateTarefaAdmin(id, num, questao, escolhas, explicacao, true, false);
    ...
    function insereExercicio(){
        ...
        if (validaCampos()){
            ...
            var idd = $('#tipo-id-tarefa').val();
            var idsender = Date.now();
            if (idd != ''){
                idsender = idd.replace('questao-', '');
            }
            addQuestao(idsender, questao, escolhas, explicacao, idd);
            resetExercicio();
        }
    }
    ...
    function initializeQuill(){
        esctar = new Quill('#questao-tarefas', {
        ...
        exptar = new Quill('#explica-tarefas', {
    ...
    function addEscolha(value="", correta=false) {
        ...
        $('#add_exercicio .label-erro').remove();
        numEsc++;
        $('#escolhas-tarefas').append(htmlEscolha(id, value, correta));
    }
</script>