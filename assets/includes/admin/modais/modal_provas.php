...
<script>
    var totalNotas = document.getElementById('nota_maxima').value != '' ? parseFloat(document.getElementById('nota_maxima').value) : 0;
    ...
    function templateprovaAdmin(id, num, questao, respostas, notaAtual, inputs=false, edit=false){
        var respostas_html = '';
    ...
    function addQuestao(id, questao, escolhas, idd, notaAtual){
        if (idd != ''){
            var sendnum = $(idd + ' .card-title').html().replace('Questão ', '');
            var senderId = idd.replace('#questao-', '');
            var template = templateprovaAdmin(senderId, sendnum, questao, escolhas, notaAtual, true, true);
            $(idd).html(template);
        } else {
            var num = $('.questoes').length + 1;
            var template = templateprovaAdmin(id, num, questao, escolhas, notaAtual, true, false);
    ...
    function openExercicio(id = ""){
        if (id != ""){
            ...
            $('#val-nota-provas').val($(id+' .notaAt').val());
    ...
    function insereExercicio(){
        ...
        if (idd != ''){
            ...
            busca = busca.replace('#', '');
            var notaEdit = $('#val_nota_'+busca).val();
            totalNotas -= parseFloat(notaEdit);
            reducing = parseFloat(notaEdit);
        }
        if (validaCampos()){
            ...
            addQuestao(idsender, questao, escolhas, idd, notaAtual);
            resetExercicio();
        } else {
            totalNotas += reducing;
        }
